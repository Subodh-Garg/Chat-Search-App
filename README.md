...
####App flow 
```Login -> Home -> Chat Search -> Home -> My Favorites```

####App Usage
1. **Login** 
 * Fill email and password text fields -> Register(if new user) -> Login (or you can directly login using already created user :  *email*: gargsubo@gmail.com, *password*: 1234 ) "*OR*"  Login using facebook.
 * Handles different error cases like 'email/password not provided', 'wrong email/password' etc.  
2. **Home**
 * Displays option for Chat Search.  
3. **Chat Search** 
